...
enum QRMode { MODE_NUMBER= 1 << 0, MODE_ALPHA_NUM= 1 << 1, MODE_8BIT_BYTE= 1 << 2, MODE_KANJI= 1 << 3 };

enum QRErrorCorrectLevel { L = 1, M = 0, Q = 3, H = 2 };

enum QRMaskPattern { PATTERN000= 0, PATTERN001= 1, PATTERN010= 2, PATTERN011= 3, PATTERN100= 4, PATTERN101= 5, PATTERN110= 6, PATTERN111= 7 };
...
class QRCode {
    ...
    public constructor(userOptions: any) {
        ...
        if (userOptions) {
		    for (var i in (userOptions as Options)) {
			    // @ts-ignore
                this.options[i] = userOptions[i];
		    }
	    }

        if (this.options.content.length === 0 || this.options.content.length > 7000) { throw new Error("Expected 'content' to be non-empty and less than 6K !"); }
    ...
    public svg():string {
    	...
    	for (var y = 0; y < length; y++) {
            for (var x = 0; x < length; x++) {
                ...
                if (module) {
                    var px: number|string = x * xsize + this.options.padding * xsize;
                    var py: number|string = y * ysize + this.options.padding * ysize;
    
                    if (join) {
                        //Module as a part of svg path data, thanks to @danioso
                        var w: number|string = xsize + px;
                        var h: number|string = ysize + py;
